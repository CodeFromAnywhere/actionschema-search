...
// 2. add caching somehow (can be done with crud or maybe vercel-native?)
// 3. add force refresh parameter: &force=1
// 4. host this for free at `site.actionschema.com/?openapiUrl=xxxx`
...
export const GET = async (request: Request) => {
  ...
  const openapiString = await fetch(openapiUrl).then((res) => res.text());

  console.log({ openapiStringLength: openapiString.length });

  const systemPrompt = `<artifacts_info>
  ...
  const prompt = `This is the API description of ${openapiUrl}:
  
  ${openapiString}

  Create a website. You MUST come out with a creative use case for this API and build the website accordingly with the endpoints. Connect the endpoints with the frontend code. Afterwards, create a skelleton using only html, css, and javascript. Be creative, create images, lists, tabs, colors. Include the code beetween <antArtifact><antArtifact/>.`;
  ...
  const regex = /<antArtifact\b[^>]*>(.*?)<\/antArtifact>/gs;
  const matches = chatResponse.matchAll(regex);
  ...
  // const c = await fs.writeFile('public/example.html', code);
