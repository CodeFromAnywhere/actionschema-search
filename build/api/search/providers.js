...
export const GET = async (request) => {
    ...
    let text;
    if (q) {
        text = await fetch(url.origin + `/audio/actionmap?q=${encodeURIComponent(q)}`).then((res) => res.text());
    ...
    return new Response(JSON.stringify({ result, createdAt: Date.now(), version: 1, text, json }), {
        status: 200,
