...
export const getOperationsForAction = async (context) => {
    ...
    const providers = await fetch(origin + "/providers.json").then((res) => res.json());
...
{ operations: {
  ...
  likelihood:number
    ...
    const sorted = tryParseJson(groqOperationArrayString)?.operations?.sort((a, b) => b.likelihood - a.likelihood);
    return { operations: sorted };
};
//# sourceMappingURL=getOperationsForAction.js.map